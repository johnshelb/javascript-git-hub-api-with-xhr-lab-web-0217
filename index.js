...
function getRepositories(){
  ...
  const uri = rootUri + "/users/" + name + "/repos"
...
function getCommits(x){
  ...
  const uri = rootUri + "/repos/" + x.dataset.username + "/" + repoName + "/commits"
...
function getBranches(x){
  const repoName = x.dataset.repository
  const uri = rootUri + "/repos/" + x.dataset.username + "/" + repoName + "/branches"
  const xhr = new XMLHttpRequest()
